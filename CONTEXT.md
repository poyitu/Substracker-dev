...
- **syncToCalendar**: 是否同步到系统日历（默认 `false`，用户在订阅列表中手动开启）
...
移动端 App 将开启了 `syncToCalendar` 的 Subscription 的 ReminderRule 转化为手机系统日历事件的过程。策略：独立日历 + 全量替换（删除该日历下所有事件后重新写入）。支持全局闹钟提前提醒（用户可设 0~N 分钟）。
